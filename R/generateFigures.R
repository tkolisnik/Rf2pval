...
#' @export
weightRankPlot<-function(truevalues,permutatedvalues,quantiledata,xlimitmin=0,xlimitmax=500,ylimitmin=0,ylimitmax=0.0045){
  pdata<-quantiledata
  wrplot<- pdata %>%
    dplyr::filter(mean > 0) %>%
    ...
    xlab("Feature Rank")+
    ylab("Feature Weight")+
    geom_line(data = permutatedvalues %>% dplyr::filter(featureWeight > 0),
...
#' @export
logWeightRankPlot<-function(truevalues,permutatedvalues,quantiledata,xlimitmin=1,xlimitmax=500,ylimitmin=-10,ylimitmax=0,labelverticaladjust=2,labelhorizontaladjust=0){
  pdata<-quantiledata
  lwrplot<- pdata %>%
    dplyr::filter(mean > 0) %>%
    ...
    geom_line(data = permutatedvalues %>% dplyr::filter(LogfeatureWeight > 0),
...
#' This function generates a histogram illustrating the sum of absolute deviations in the permutated set vs the true (observed) set. It is a visualization of the data used to calculate the p-value for the entire feature set.
#'
#' @param permutatedvalues a tibble of featureRanks, feature names, feature weights, log feature weights and permutation # generated by the load_permutated_fws function
#' @param quantiledata a tibble containing: featureRanks, mean, lower, upper, and observed as well as logmean, loglower, logupper, logobserved generated by the calculateQuantiles function
#' @return A histogram of count by sum of absolute deviations, showing the differences between the null and observed datasets.
#' @export
piHistogram<-function(permutatedvalues,quantiledata){
    d<-permutatedvalues
