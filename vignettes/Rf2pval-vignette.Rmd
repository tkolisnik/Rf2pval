---
title: "Rf2pval: A comprehensive tool for genomic data analysis featuring scikit-learn Random Forests in R"
output: 
...
-   Have a genomic data (RNA-seq or Microarray) with a binary outcome variable.
...
#pythonhome:     /opt/homebrew/Caskroom/miniconda/base/envs/rf2pval-conda:/.../rf2pval-conda
#version:        3.9.18 | packaged by conda-forge | (main, Aug 30 2023, 03:53:08)  [Clang 15.0.7 ]
#numpy:          /opt/homebrew/Caskroom/miniconda/base/envs/rf2pval-conda/lib/python3.9/sit...
#numpy_version:  1.26.0
#sklearn:        /opt/homebrew/Caskroom/miniconda/base/envs/rf2pval-conda/lib/python3.9/sit...
...
```{r, eval=FALSE}
data("demo_rnaseq_data", package = "Rf2pval")
```
...
<br> <br> <strong>Example data structures expected by Rf2pval:</strong><br> *These are tibbles which should be saved into one list variable*
...
processed_training_data <- Rf2pval::create_feature_matrix(
  dataset = demo_rnaseq_data$training_data, 
  set_type = "training"
  )

processed_validation_data <- Rf2pval::create_feature_matrix(
  dataset = demo_rnaseq_data$validation_data, 
  set_type = "validation"
  )

processed_testing_data <- Rf2pval::create_feature_matrix(
...
tuning_results <- Rf2pval::tune_and_train_rf_model(
...
custom_parameter_grid <- list(
  ...
  # It is not recommended to change criterion, as gini is required by Rf2pval functions.
  criterion = list("gini"), 
...
# Train a model and perform hyperparameter tuning using Rf2pval. 
...
fitting_results <- Rf2pval::fit_and_evaluate_rf(
...
fitting_results_testing <- Rf2pval::fit_and_evaluate_rf(
                                      best_params = tuning_results$best_params, 
                                      X_train = processed_training_data$X_training_mat, 
                                      y_train = processed_training_data$y_training_vector,
...
feat_importances <- Rf2pval::calculate_feature_importances(
...
quantile_data <- Rf2pval::calculate_quantiles(
...
pvalue_set <- Rf2pval::calculate_full_set_pvalue(
                        permutedvalues = feat_importances$permuted_importances, 
...
pvalues_ranks <- Rf2pval::calculate_ranked_based_pvalues(
                        truevalues = feat_importances$true_importances, 
                        permutedvalues = feat_importances$permuted_importances, 
                        alpha = 0.05
                        )
...
fiplot_full <- Rf2pval::generate_fi_rank_plot(
  permutedvalues = feat_importances$permuted_importances, 
  quantiledata = quantile_data, 
  xlimitmin = 1, 
  xlimitmax = 10, 
...
fiplot_focused <- Rf2pval::generate_fi_rank_plot(
  permutedvalues = feat_importances$permuted_importances, 
  quantiledata = quantile_data, 
  xlimitmin = 1, 
  xlimitmax = 10, 
...
pihist_plot <- Rf2pval::generate_pi_histogram(
  permutedvalues = feat_importances$permuted_importances, 
  quantiledata = quantile_data
  )
...
ecdf_plot <- Rf2pval::generate_pi_ECDF_plot(
  permutedvalues = feat_importances$permuted_importances,
  quantiledata = quantile_data)
...
Explore advanced functionalities of Rf2pval such as SHAP value analysis, or use it to plug into additional gene ontology, enrichment and gprofiler modules.
</div>
...
shapvals <- Rf2pval::calculate_SHAP_values(
  model = fitting_results$model, 
...
shapplot <- Rf2pval::generate_shap_plots(
...
<p>
  Rf2pval provides comprehensive tools for bioinformatics analysis, including features for machine learning, feature importance evaluation, and more. This vignette covers the basics to get you started. For more detailed documentation on the use of each function, please refer to the function help pages within the package using the <code>?Rf2pval</code> feature.
...
<br>
  ...
  Webpage: [www.github.com/tkolisnik/Rf2pval](https://www.github.com/tkolisnik/Rf2pval) &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; Contact: <tkolisnik@gmail.com>
</p>
